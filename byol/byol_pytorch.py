...
class BYOL(nn.Module):
    def __init__(self, net, image_size, hidden_layer = -2, projection_size = 256, projection_hidden_size = 4096, augment_fn = None, augment_fn2 = None, moving_average_decay = 0.99):
        ...
        DEFAULT_AUG = nn.Sequential(
            ...
            #augs.Normalize(mean=torch.tensor([0.485, 0.456, 0.406]), std=torch.tensor([0.229, 0.224, 0.225])) # Imagenet
            augs.Normalize(mean=torch.tensor([0.5444, 0.3120, 0.2602]), std=torch.tensor([0.2240, 0.1733, 0.1637])) # Keratitis
        )
        ...
        self.forward(self.augment1(dummy), self.augment2(dummy))
